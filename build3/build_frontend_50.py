...
# Einheitliche Versionsnummern fuer Frontend 5.0.2. Backend 5.0.1 bleibt kompatibel.
for old in ("version:'3.9'","version:'5.0'","version:'5.0.1'"):
    js=js.replace(old,"version:'5.0.2'")
for old in ("DG_APP_VERSION='3.9'","DG_APP_VERSION='5.0'","DG_APP_VERSION='5.0.1'"):
    js=js.replace(old,"DG_APP_VERSION='5.0.2'")
for old in ("clientVersion:'3.9'","clientVersion:'5.0'","clientVersion:'5.0.1'"):
    js=js.replace(old,"clientVersion:'5.0.2'")
js=js.replace('App 3.9','App 5.0.2').replace('App 5.0.1','App 5.0.2').replace('Google-GS 3.0','Google-GS 5.0.1').replace('Backend 3.0','Backend 5.0.1')
js=js.replace('/^3\\./','/^(?:3\\.|5\\.)/')
css=css+'\n'+feat_css+'\n'
(O/'app-5.0.js').write_text(js);(O/'app-5.0.css').write_text(css)
html=BeautifulSoup(base_html.read_text(),'html.parser');html.title.string='DG Zeiterfassung 5.0.2';login=html.select_one('.login-card .center.muted.small');hero=html.select_one('.hero .head-row strong')
if login:login.string='Version 5.0.2'
if hero:hero.string='Zeiterfassung - 5.0.2'
