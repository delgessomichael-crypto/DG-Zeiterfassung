...
function cleanup10(){['adminTimeBankEmployee','adminTimeBankBalance','adminTimeBankHours','adminTimeBankReason','adminTimeBankAction','adminTimeBankStatus'].forEach(id=>q(id)?.classList.add('dg10-hide'));document.querySelectorAll('option').forEach(o=>{const t=String(o.textContent||'').trim();if(t==='Freizeitausgleich'||t==='Unentschuldigte Abwesenheit')o.remove();});const sel=q('absenceType');if(sel&&!Array.from(sel.options).some(o=>o.value==='Unerlaubte Abwesenheit')){const o=document.createElement('option');o.value='Unerlaubte Abwesenheit';o.textContent='Unerlaubte Abwesenheit';sel.appendChild(o);}}
