...
for p in [base,base_css,base_html,base_manifest]:
    if not p.exists(): raise SystemExit('stable 3.5.2 base missing: '+str(p))
...
js=js.replace("version:'3.5.2'","version:'3.5.3'",1).replace("DG_APP_VERSION='3.5.2'","DG_APP_VERSION='3.5.3'",1).replace("clientVersion:'3.5.2'","clientVersion:'3.5.3'",1).replace('App 3.5.2','App 3.5.3')
(O/'app-3.5.js').write_text(js);(O/'app-3.5.css').write_text(css)
html=BeautifulSoup(base_html.read_text(),'html.parser');html.title.string='DG Zeiterfassung 3.5.3';login=html.select_one('.login-card .center.muted.small');hero=html.select_one('.hero .head-row strong')
if login:login.string='Version 3.5.3'
if hero:hero.string='Zeiterfassung - 3.5.3'
