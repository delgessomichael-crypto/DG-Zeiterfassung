...
js=base_js.read_text();css=base_css.read_text();feat=(R/'src/features_3_8.js').read_text()
...
js=js.replace("version:'3.7.1'","version:'3.8'",1).replace("DG_APP_VERSION='3.7.1'","DG_APP_VERSION='3.8'",1).replace("clientVersion:'3.7.1'","clientVersion:'3.8'",1).replace('App 3.7.1','App 3.8')
(O/'app-3.5.js').write_text(js);(O/'app-3.5.css').write_text(css)
html=BeautifulSoup(base_html.read_text(),'html.parser');html.title.string='DG Zeiterfassung 3.8';login=html.select_one('.login-card .center.muted.small');hero=html.select_one('.hero .head-row strong')
if login:login.string='Version 3.8'
if hero:hero.string='Zeiterfassung - 3.8'
