...
function d32GalleryShow(){const m=$('d32Gallery'),g=DG3.gallery;if(!m||!g?.items?.length)return;g.index=(g.index+g.items.length)%g.items.length;const x=g.items[g.index],src=x.url||d32Thumb(x.id,'w1600');m.querySelector('.d32-gallery-image').src=src;m.querySelector('.d32-gallery-count').textContent='Bild '+(g.index+1)+' von '+g.items.length;const a=m.querySelector('.d32-gallery-original');a.href=src;a.classList.toggle('hidden',!a.href);}
...
d3ReportCard=function(g,view,index){
  ...
  const grid=d3Element('div','d32-photo-grid');labels.forEach((old,i)=>{const cb=old.querySelector('.d3-photo');const item=d3Element('div','d32-photo-item');const check=document.createElement('label');check.className='d32-photo-check';check.append(cb,document.createTextNode(' Bild '+(i+1)));const b=document.createElement('button');b.type='button';b.className='d32-thumb';b.title='Bild '+(i+1)+' vergrößern';const img=document.createElement('img');img.src=items[i].url||d32Thumb(items[i].id);img.alt='Vorschau Bild '+(i+1);img.loading='lazy';img.onerror=()=>{const fallback=d32Thumb(items[i].id);if(fallback&&img.src!==fallback)img.src=fallback;};b.append(img);b.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();d32GalleryOpen(items,i);});item.append(b,check);grid.append(item);old.remove();});
