...
for p in [base_js,base_css,base_html,base_manifest]:
    if not p.exists(): raise SystemExit('stable 3.8 base missing: '+str(p))
js=base_js.read_text();css=base_css.read_text();feat=(R/'src/features_3_9.js').read_text();feat_css=(R/'src/features_3_9.css').read_text()
...
js=js.replace("version:'3.8'","version:'3.9'",1).replace("DG_APP_VERSION='3.8'","DG_APP_VERSION='3.9'",1).replace("clientVersion:'3.8'","clientVersion:'3.9'",1).replace('App 3.8','App 3.9')
css=css+'\n'+feat_css+'\n'
(O/'app-3.5.js').write_text(js);(O/'app-3.5.css').write_text(css)
html=BeautifulSoup(base_html.read_text(),'html.parser');html.title.string='DG Zeiterfassung 3.9';login=html.select_one('.login-card .center.muted.small');hero=html.select_one('.hero .head-row strong')
if login:login.string='Version 3.9'
if hero:hero.string='Zeiterfassung - 3.9'
