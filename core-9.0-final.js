...
function boot522(){addCss522();installRenderer522();installMonthSelectors522();document.title='DG Zeiterfassung 10.0';document.querySelectorAll('.login-card .muted.small').forEach(x=>{if(/^Version /.test((x.textContent||'').trim()))x.textContent='Version '+V522});document.querySelectorAll('.hero strong').forEach(x=>{if(/Zeiterfassung/.test(x.textContent||''))x.textContent='Zeiterfassung - '+V522});try{if(window.DG3)DG3.version=V522;window.DG_APP_VERSION=V522}catch(_e){}setTimeout(installMonthSelectors522,500)}
