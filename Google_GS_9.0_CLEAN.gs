...
function doPost(e) {
  ...
  try {
    ...
    if (action === 'addMaintenanceRepair') {
      return jsonResponse_({ok:true,data:addMaintenanceRepair37_(clean_(data.employee),clean_(data.employeePin),clean_(data.deviceId),clean_(data.date),clean_(data.description),clean_(data.repairId))});
    }
    ...
    if (action === 'addManualMaintenanceCount') {
      return jsonResponse_({ok:true,data:addManualMaintenanceCount502_(clean_(data.employee),clean_(data.employeePin),clean_(data.date),Number(data.count)||0,clean_(data.note),clean_(data.manualId))});
    }
...
function addMaintenanceRepair37_(employee,pin,deviceId,date,description,repairId){requireChef_(employee,pin);ensureMaintenanceSchema37_();const d=findMaintenanceDevice37_(deviceId);if(!d||!d.active)throw new Error('Wartungsgerät nicht gefunden.');if(!validDate3_(date))throw new Error('Reparaturdatum ist ungültig.');if(!clean_(description))throw new Error('Reparaturbeschreibung fehlt.');const sh=getSpreadsheetRaw_().getSheetByName('Wartungsreparaturen'),id=clean_(repairId)||('WR-'+Utilities.getUuid());sh.appendRow([id,d.id,d.customerId,d.objectId,date,clean_(description),new Date(),employee]);return{id:id};}
...
function addManualMaintenanceCount502_(employee,pin,date,count,note,manualId){requireChef_(employee,pin);ensureMaintenanceSchema37_();date=clean_(date)||Utilities.formatDate(new Date(),CONFIG.TZ,'yyyy-MM-dd');if(!validDate3_(date))throw new Error('Datum ist ungültig.');count=Math.floor(Number(count)||0);if(count<1||count>99)throw new Error('Bitte eine Anzahl zwischen 1 und 99 eintragen.');const sh=getSpreadsheetRaw_().getSheetByName('Wartungsmanuell'),id=clean_(manualId)||('WM-'+Utilities.getUuid());sh.appendRow([id,date,count,clean_(note),new Date(),employee]);return{id:id,date:date,count:count,note:clean_(note)};}
