const CACHE_NAME='dg-zeiterfassung-5-2-9-office-20260916a';
const OFFER_PATCH='./app-5.2.6-offers.js?v=20260916b';
const REGIE_PATCH='./app-5.2.7-regie.js?v=20260916a';
const OFFICE_PATCH='./app-5.2.9-office.js?v=20260916a';
...
async function dg529PatchedHtml(response){
  ...
  if(!html.includes('app-5.2.9-office.js'))html=html.replace('</body>','<script defer src="'+OFFICE_PATCH+'"></script></body>');
...
if(e.request.mode==='navigate'){e.respondWith((async()=>{try{return await dg529PatchedHtml(await fetch(e.request,{cache:'no-store'}));}catch(_){const c=await caches.open(CACHE_NAME),hit=await c.match(new URL('./index.html',base).href)||await c.match(new URL('./',base).href);return hit?await dg529PatchedHtml(hit):Response.error()}})());return;}
