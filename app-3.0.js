...
function renderWeek(){const ds=visibleDays(),out=$('dg62Week');if(!out)return;out.innerHTML='';const end=add(weekStart,5);$('dg62WeekTitle').textContent='Woche '+fmtShort(weekStart)+' bis '+fmtShort(end);if(!ds.length){out.innerHTML='<div class="status info">Für diese Woche sind keine verbleibenden Tage mehr anzuzeigen.</div>';return}ds.forEach(d=>out.appendChild(renderDay(d)))}
window.dg62Load=async function(){if(!navigator.onLine){status('dg62Status','Offline – Kalender kann nicht synchronisiert werden.','warn');return}try{status('dg62Status','Kalender wird synchronisiert …');const end=add(weekStart,5),r=await Promise.all([api(chefPayload({action:'getPlannerWorkers'})),api(chefPayload({action:'getPlannerEvents',startDate:weekStart,endDate:end}))]);workers=r[0]||[];events=r[1]||[];renderWorkers();renderWeek();if(DG3.enhanceCalendar)DG3.enhanceCalendar();lastSync=new Intl.DateTimeFormat('de-DE',{hour:'2-digit',minute:'2-digit'}).format(new Date());if($('dg62LastSync'))$('dg62LastSync').textContent='Zuletzt synchronisiert: '+lastSync+' Uhr';status('dg62Status','✓ Kalender synchronisiert.','ok')}catch(e){status('dg62Status',e&&e.message?e.message:'Kalender konnte nicht geladen werden.','error')}};
window.dg62ThisWeek=function(){weekStart=monday();if($('dg62Date'))$('dg62Date').value=weekStart;dg62Load()};
window.dg62PrevWeek=function(){weekStart=add(weekStart,-7);$('dg62Date').value=weekStart;dg62Load()};
window.dg62NextWeek=function(){weekStart=add(weekStart,7);$('dg62Date').value=weekStart;dg62Load()};
window.dg62GoWeek=function(d){if(!d)return;weekStart=monday(d);$('dg62Date').value=weekStart;dg62Load()};
window.dg62JumpToday=async function(){weekStart=monday();$('dg62Date').value=weekStart;await dg62Load();setTimeout(()=>{const x=document.querySelector('.dg62-day.today');if(x)x.scrollIntoView({behavior:'smooth',block:'start'})},100)};
...
function d3Open(id,child){const c=$(id);if(!c)return;[...$('bossView').children].filter(x=>x.classList.contains('card')).forEach(x=>d3Collapse(x,x!==c));if(child){c.querySelectorAll('.d3-panel').forEach(x=>x.classList.toggle('hidden',x.id!==child));c.querySelectorAll('[data-panel]').forEach(x=>x.setAttribute('aria-selected',String(x.dataset.panel===child)));}DG3.open=child||id;const fn=DG3.loaders[DG3.open];if(fn)Promise.resolve(fn()).catch(e=>d3Notice(e.message,'error'));}
...
function d3InstallOffice(){const root=$('bossView'),cards=[...root.children],find=t=>cards.find(c=>(c.querySelector(':scope > .dg48-head h2,:scope > h2')?.textContent||'').includes(t));const completed=find('Regieberichte');completed.id='d3Completed';completed.querySelector('h2').textContent='Abgeschlossene Auftr\u00e4ge';$('regieMergeToolbar')?.remove();
 ...
 const details=planner.querySelector('.dg62-main');details.addEventListener('toggle',()=>{if(details.open){[...root.children].filter(x=>x.classList.contains('card')&&x!==planner).forEach(x=>d3Collapse(x,true));DG3.open=planner.id;dg62Load();}});
