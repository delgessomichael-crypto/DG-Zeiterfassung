...
function renderBossActions(){
  ...
  let html=bossToolbar(x)+'<div class="action-tabs"><button class="btn '+(bossDetailTab==='overview'?'primary':'secondary')+'" onclick="return setBossDetailTab(\'overview\')">Monatsdetails</button><button class="btn '+(bossDetailTab==='entries'?'primary':'secondary')+'" onclick="return setBossDetailTab(\'entries\')">Aufträge</button><button class="btn '+(bossDetailTab==='alerts'?'primary':'secondary')+'" onclick="return setBossDetailTab(\'alerts\')">Auffälligkeiten</button><button class="btn '+(bossDetailTab==='adjust'?'primary':'secondary')+'" onclick="return setBossDetailTab(\'adjust\')">± Stunden</button><button class="btn '+(bossDetailTab==='closure'?'primary':'secondary')+'" onclick="return setBossDetailTab(\'closure\')">Monatsabschluss</button></div>';
...
function renderBossOverview(x){const b=Number(x.balance||0);return '<div class="status info"><strong>Soll '+formatHours(x.targetTotal)+' Std. · Ist '+formatHours(x.actualTotal)+' Std. · '+(b>=0?'+':'')+formatHours(b)+' Std.</strong></div><div class="entry">Arbeitszeit '+formatHours(x.workTotal)+' Std. · Gutschriften '+formatHours(x.creditedTotal)+' Std. · Korrekturen '+(Number(x.adjustmentTotal)>0?'+':'')+formatHours(x.adjustmentTotal)+' Std.</div><div class="entry">Monat: Urlaub '+Number(x.vacationDays||0)+' · Krank '+Number(x.sickDays||0)+' · Feiertage '+Number(x.holidayDays||0)+'</div><div class="entry">Jahr: Urlaub beansprucht '+Number(x.yearVacationDays||0)+' · zustehend '+formatHours(x.vacationEntitlement)+' · Rest '+formatHours(x.vacationRemaining)+' Tage</div><div class="entry">Für Abrechnung: '+formatHours(x.payableHours||0)+' Std.</div><div class="entry">Prüfstatus: '+bossTraffic(x)+' · '+bossOpenIssues(x)+' offene Punkte · Monatsabschluss: <span class="closure-badge '+(x.closureStatus==='Abgeschlossen'?'closed':'')+'">'+esc(x.closureStatus||'Offen')+'</span></div>'}
...
function renderBossTimeBank(){return ''}
...
window.loadBossDayClosuresV48=async function(){if(!navigator.onLine){setMessage('dg48DayStatus','Tagesabschlüsse benötigen Internet.','warn');return}const year=Number(el('dg48DayYear').value),month=Number(el('dg48DayMonth').value);try{setMessage('dg48DayStatus','Tagesabschlüsse werden geladen ...','info');const rows=await api(chefPayload({action:'getBossDayClosures',year:year,month:month,force:true}));renderBossDayClosuresV48(rows||[]);setMessage('dg48DayStatus','✓ Tagesabschlüsse geladen.','ok')}catch(e){setMessage('dg48DayStatus',e.message,'error')}};
...
window.renderBossDayClosuresV48=function(rows){
    ...
    out.innerHTML=rows.map(function(emp,ei){
      return '<div class="dg48-days-employee"><strong>'+esc(emp.employee)+'</strong><div class="dg48-day-grid">'+(emp.days||[]).map(function(d,di){
        var status=String(d.status||'Arbeiten'),statusDay=status!=='Arbeiten',unexcused=status==='Unerlaubte Abwesenheit'||status==='Unentschuldigte Abwesenheit';
        var credit=unexcused?0:Number(d.creditedHours||0),c=statusDay?true:!!d.closed,key=ei+'_'+di;
        ...
        var statusBadge=statusDay?' <span class="dg72-status-badge">'+esc(status)+'</span>':'';
        var stateText=unexcused?'🔴 Unerlaubte Abwesenheit':(statusDay?'🔵 '+esc(status)+' · automatisch abgeschlossen':(c?'🟢 Vollständig übertragen':'🔴 Noch nicht abgeschlossen'));
        var meta=statusDay?(esc(formatHours(credit))+' Std. Gutschrift · '+Number(d.entryCount||0)+' Arbeitsbericht'+(Number(d.entryCount||0)===1?'':'e')):(esc(formatHours(d.hours||0))+' Std. · '+Number(d.entryCount||0)+' Eintrag/Einträge');
        var statusInfo=statusDay?'<div class="dg72-report-status">'+esc(status)+' · '+esc(formatHours(credit))+' Std. automatisch '+(credit>0?'gutgeschrieben':'ohne Gutschrift')+' · Tag automatisch abgeschlossen</div>':'';
        return '<div class="dg48-day '+(c?'closed':'open')+(statusDay?' dg-status-day':'')+(unexcused?' dg-status-unexcused':'')+'" onclick="return toggleBossDayDetailsV49(\''+key+'\')"><div class="dg48-day-state '+(unexcused?'open':(c?'closed':'open'))+'">'+stateText+'</div><strong>'+esc(formatDateDE(d.date))+'</strong>'+statusBadge+'<div class="muted small">'+meta+'</div><div class="dg49-hint">Berichtdetails anzeigen</div><div id="dg49Detail_'+key+'" class="dg49-detail hidden">'+statusInfo+detail+((!c&&!statusDay)?'<button type="button" class="btn danger" data-employee="'+esc(emp.employee)+'" data-date="'+esc(d.date)+'" onclick="return event.stopPropagation();manualCloseBossDayV48(this.dataset.employee,this.dataset.date)">Tag manuell abschließen</button>':'')+'</div></div>';
      }).join('')+'</div></div>';
    }).join('');
  };
addStyles();
...
function calendarRange(){
  ...
  const count=calendarMode==='week'?6:14;
...
function d3ReportCard(g,view,index){const billed=view==='Abgerechnet',ids=[...new Set((g.objectIds||[g.objectId]).filter(Boolean))],reports=(g.reports||[]).filter(x=>x.id),key=view+':'+index,c=d3Element('div','report-card '+(billed?'billed':'open')+(index%2?' d3-alt':''));c.dataset.index=index;c.dataset.view=view;window.__dgGroupMap[key]=reports.map(x=>String(x.id));reports.forEach(r=>window.__dgReportMap[String(r.id)]=r);const state=billed?'Abgerechnet':view==='Laufend'?'Laufend':'Abgeschlossen';c.innerHTML=(!billed?'<label class="d3-selection"><input type="checkbox" class="regie-merge-select" data-index="'+index+'" data-object-ids="'+esc(ids.join(','))+'"> Zum Zusammenf\u00fchren markieren</label>':'')+'<div class="d3-head"><strong>\ud83c\udfe2 '+esc(g.customer)+'</strong><span class="job-status '+(billed?'billed':view==='Laufend'?'running':'completed')+'">'+state+'</span></div><div class="report-meta">'+esc(formatDateDE(g.firstDate))+(g.lastDate!==g.firstDate?' bis '+esc(formatDateDE(g.lastDate)):'')+' - '+reports.length+' Bericht(e)</div><div class="total">Gesamt Personal: '+formatHours(g.totalHours)+' Std.</div><div class="report-meta"><strong>Mitarbeiter:</strong> '+esc((g.employees||[]).join(', '))+'</div><details><summary>Einzelberichte anzeigen</summary>'+reports.map(d3Single).join('')+'</details>';
 const photos=new Map();reports.forEach(r=>{String(r.photoFileIds||'').split(',').map(x=>x.trim()).filter(id=>/^FILE-/i.test(id)).forEach(id=>photos.set(id,'https://dg-app-10-api-production.up.railway.app/v1/files/'+encodeURIComponent(id)));});c.append(d3Element('div','d3-export','<strong>Bericht herunterladen</strong><div class="muted small">Exportiert genau die '+reports.length+' angezeigten Einzelberichte mit Unterschriften und ausgew\u00e4hlten Bildern.</div>'+[...photos].map(([id,u],i)=>'<label class="d3-selection"><input type="checkbox" class="d3-photo" value="'+esc(id)+'" checked> Bild '+(i+1)+' <a target="_blank" rel="noopener" href="'+esc(u)+'">ansehen</a></label>').join('')+d3Button('Bericht herunterladen','d3Export',[view,index])+'<div class="d3-export-status"></div>'));
...
async function d3InquiryOffer(id){
  const r=d3Inquiry(id);
  if(!confirm('Diese Kundenanfrage nach „Zu erstellende Angebote“ verschieben?'))return;
  const created=await api(chefPayload({action:'inquiryToOffer',id,customer:r.customer||'',phone:r.phone||''}));
  const offerId=String(created&&created.offerId||'').trim();
  if(!offerId)throw new Error('Angebot konnte nicht eindeutig erstellt werden.');
  await api(chefPayload({action:'moveOfferBackToCreate',offerId}));
  await d3Inquiries();
  if(typeof d3Dashboard==='function')await d3Dashboard(true);
}
...
function d32Thumb(id,size='w240'){id=String(id||'').trim();if(/^FILE-/i.test(id))return 'https://dg-app-10-api-production.up.railway.app/v1/files/'+encodeURIComponent(id);return 'https://drive.google.com/thumbnail?id='+encodeURIComponent(id)+'&sz='+size;}
...
function d32GalleryShow(){const m=$('d32Gallery'),g=DG3.gallery;if(!m||!g?.items?.length)return;g.index=(g.index+g.items.length)%g.items.length;const x=g.items[g.index],src=d32Thumb(x.id,'w1600');m.querySelector('.d32-gallery-image').src=src;m.querySelector('.d32-gallery-count').textContent='Bild '+(g.index+1)+' von '+g.items.length;const a=m.querySelector('.d32-gallery-original');a.href=src;a.classList.toggle('hidden',!a.href);}
...
d3ReportCard=function(g,view,index){
  ...
  const grid=d3Element('div','d32-photo-grid');labels.forEach((old,i)=>{const cb=old.querySelector('.d3-photo');const item=d3Element('div','d32-photo-item');const check=document.createElement('label');check.className='d32-photo-check';check.append(cb,document.createTextNode(' Bild '+(i+1)));const b=document.createElement('button');b.type='button';b.className='d32-thumb';b.title='Bild '+(i+1)+' vergrößern';const img=document.createElement('img');const previewSrc=d32Thumb(items[i].id);img.src=previewSrc;img.alt='Vorschau Bild '+(i+1);img.loading='eager';img.decoding='async';img.onerror=()=>{const fallback=items[i].url||'';if(fallback&&img.src!==fallback)img.src=fallback;};b.append(img);b.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();d32GalleryOpen(items,i);});item.append(b,check);grid.append(item);old.remove();});
...
function d35InstallInspectionButton(){const b=$('d35InspectionBtn');if(b)b.remove();}
...
function versionLabels520(){document.title='DG Zeiterfassung 10.0';document.querySelectorAll('.hero strong').forEach(x=>{if(/Zeiterfassung/.test(x.textContent))x.textContent='Zeiterfassung - 5.2.0'});document.querySelectorAll('.login-card .muted.small').forEach(x=>{if(/^Version /.test(x.textContent.trim()))x.textContent='Version 5.2.0'});try{DG3.version=V520;window.DG_APP_VERSION=V520}catch(_e){}try{sessionStorage.removeItem('dg51_backend')}catch(_e){}}
...
function boot521(){applyFixes521();const root=q521('bossView');if(root)observer521.observe(root,{childList:true,subtree:true});setTimeout(applyFixes521,250);setTimeout(applyFixes521,1200);setInterval(ensurePayrollTile521,3600000);document.title='DG Zeiterfassung 10.0';document.querySelectorAll('.login-card .muted.small').forEach(x=>{if(/^Version /.test((x.textContent||'').trim()))x.textContent='Version '+V521});document.querySelectorAll('.hero strong').forEach(x=>{if(/Zeiterfassung/.test(x.textContent||''))x.textContent='Zeiterfassung - '+V521});try{if(window.DG3)DG3.version=V521;window.DG_APP_VERSION=V521}catch(_e){}}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot521);else boot521();
})();

/* DG Zeiterfassung 5.2.4 - jede Auffaelligkeit kann nach manueller Pruefung freigegeben werden */
(function(){
'use strict';
const V522='5.2.4';
const $522=id=>document.getElementById(id);
const esc522=v=>String(v==null?'':v).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
let audit522={issues:[]},lastRows522=[];

function addCss522(){if($522('dg522Styles'))return;const s=document.createElement('style');s.id='dg522Styles';s.textContent=`
.dg522-review-panel{border:1px solid #dbe2ea;border-radius:14px;background:#f8fafc;padding:12px 14px;margin:0 0 14px}.dg521-collapsed .dg522-review-panel{display:none!important}.dg522-review-title{display:flex;align-items:center;gap:8px;flex-wrap:wrap;font-weight:900;margin-bottom:8px}.dg522-review-note{font-size:12px;color:#64748b;margin-bottom:8px}.dg522-review-list{display:grid;gap:8px;margin-top:8px}.dg522-issue{border-radius:11px;background:#fff;border:1px solid #e5e7eb;padding:10px 11px}.dg522-issue.error{border-left:6px solid #dc2626}.dg522-issue.warn{border-left:6px solid #f59e0b}.dg522-issue.reviewed{border-left:6px solid #16a34a!important;background:#f0fdf4}.dg522-issue-head{font-weight:900}.dg522-issue-detail{font-size:13px;color:#64748b;margin-top:3px}.dg522-issue-actions{display:flex;gap:7px;flex-wrap:wrap;margin-top:8px}.dg522-issue-actions .btn{width:auto!important;margin:0!important}.dg522-release{margin-top:12px;padding-top:10px;border-top:1px solid #dbe2ea}.dg522-release .btn{width:100%!important}.dg522-release-ok{padding:10px;border-radius:10px;background:#dcfce7;color:#166534;font-weight:900;text-align:center}@media(max-width:720px){.dg522-issue-actions .btn{width:100%!important}}
`;document.head.appendChild(s)}
function period522(){return {year:Number($522('dg48DayYear')?.value||$522('bossYear')?.value||new Date().getFullYear()),month:Number($522('dg48DayMonth')?.value||$522('bossMonth')?.value||(new Date().getMonth()+1))}}
...
function reviewedKey522(){const p=period522();return 'dg522_reviewed_'+p.year+'_'+p.month}
function reviewedSet522(){try{return new Set(JSON.parse(localStorage.getItem(reviewedKey522())||'[]'))}catch(_e){return new Set()}}
function rememberReviewed522(id){const s=reviewedSet522();s.add(id);try{localStorage.setItem(reviewedKey522(),JSON.stringify([...s]))}catch(_e){}}
function minutes522(v){const m=String(v||'').match(/^(\d{1,2}):(\d{2})$/);return m?Number(m[1])*60+Number(m[2]):null}
function normCustomer522(v){return String(v||'').trim().toLowerCase().replace(/\s+/g,' ')}
function bytesToBase64Url522(bytes){let b='';bytes.forEach(x=>b+=String.fromCharCode(x));return btoa(b).replace(/\+/g,'-').replace(/\//g,'_').replace(/=+$/,'')}
async function issueId522(parts){const text=parts.map(x=>String(x==null?'':x).trim()).join('|');if(window.crypto?.subtle){const hash=await crypto.subtle.digest('SHA-256',new TextEncoder().encode(text));return 'PAY-'+bytesToBase64Url522(new Uint8Array(hash)).slice(0,24)}let h=2166136261;for(let i=0;i<text.length;i++){h^=text.charCodeAt(i);h=Math.imul(h,16777619)}return 'DGLOCAL-'+(h>>>0).toString(16)}
async function addIssue522(list,severity,type,employee,date,title,detail,key,extra){const p=period522(),id=await issueId522([p.year,p.month,employee,date||'',type,key||'']),reviewed=reviewedSet522().has(id);list.push(Object.assign({id,severity,type,employee,date:date||'',title,detail:detail||'',reviewed},extra||{}))}
async function buildLocalIssues522(rows){
  const out=[];
  for(const emp of (rows||[])){
    const name=String(emp.employee||''),days=(emp.days||[]).slice().sort((a,b)=>String(a.date||'').localeCompare(String(b.date||'')));
    for(const day of days){
      const d=String(day.date||''),reports=day.reports||[],workReports=reports.filter(r=>Number(r&&r.hours||0)>0.0001);
      const net=Number(day.workHours!=null?day.workHours:day.hours||0),gross=Number(day.workGrossHours!=null?day.workGrossHours:workReports.reduce((s,r)=>s+Number(r.hours||0),0)),pause=Number(day.workPauseHours!=null?day.workPauseHours:day.pauseHours||0);
      if(net>10.0001)await addIssue522(out,'error','daily_over_10',name,d,'Mehr als 10 Stunden Arbeitszeit','Netto-Arbeitszeit '+net.toFixed(2).replace('.',',')+' Std.');
      ...
      if(workReports.length&&!day.closed)await addIssue522(out,'error','day_not_closed',name,d,'Tagesabschluss fehlt','Für diesen Arbeitstag wurde kein Tagesabschluss gefunden.');
      const req=net>9?0.75:(net>6?0.5:0);if(workReports.length&&req&&pause+0.0001<req)await addIssue522(out,'error','pause_short',name,d,'Pause zu kurz','Erfasst '+Math.round(pause*60)+' Min.; erforderlich mindestens '+Math.round(req*60)+' Min.');
      if(day.status&&day.status!=='Arbeiten'&&workReports.length>0&&gross>0.0001)await addIssue522(out,day.status==='Feiertag'?'warn':'error','work_and_status',name,d,'Arbeitszeit und '+day.status+' am selben Tag','Es sind echte Arbeitsstunden erfasst und der Tag ist zugleich als '+day.status+' markiert.');
    ...
    for(let i=1;i<days.length;i++){
      const prev=(days[i-1].reports||[]).slice().sort((a,b)=>String(a.end||'').localeCompare(String(b.end||''))).pop(),next=(days[i].reports||[]).slice().sort((a,b)=>String(a.start||'').localeCompare(String(b.start||'')))[0];if(!prev||!next)continue;let a=minutes522(prev.end),b=minutes522(next.start);if(a===null||b===null)continue;let rest=(1440-a+b)/60;if(rest<11)await addIssue522(out,'warn','rest_under_11',name,String(days[i].date||''),'Ruhezeit unter 11 Stunden','Zwischen '+date522(days[i-1].date)+' '+prev.end+' und '+date522(days[i].date)+' '+next.start+' liegen nur '+rest.toFixed(2).replace('.',',')+' Std.');
    }
  }
  return out;
}
function updateHeader522(box,emp,issues){const head=box?.querySelector(':scope > .dg521-employee-head');if(!head)return;const open=issues.filter(x=>!x.reviewed).length,ok=open===0;const lamp=head.querySelector('.dg521-lamp'),label=head.querySelector('.dg521-plausibility'),meta=head.querySelector('.dg521-meta');if(lamp){lamp.classList.toggle('ok',ok);lamp.classList.toggle('bad',!ok)}if(label){label.classList.toggle('ok',ok);label.classList.toggle('bad',!ok);label.textContent=ok?(issues.length?'Geprüft / freigegeben':'Alles plausibel'):'Überprüfung notwendig'}if(meta){const days=Number((emp?.days||[]).length);meta.textContent=days+' Tag'+(days===1?'':'e')+(issues.length?(open?' · '+open+' offen':' · '+issues.length+' geprüft'):'')}head.title=issues.length?issues.map(x=>(x.reviewed?'✓ ':'')+(x.date?date522(x.date)+' · ':'')+x.title).join('\n'):'Keine Auffälligkeiten in der Schnellprüfung.'}
function issueHtml522(x){const cls=x.reviewed?'reviewed':x.severity,icon=x.reviewed?'🟢':(x.severity==='error'?'🔴':'🟠'),status=x.reviewed?'geprüft – korrekt':'prüfen',emp=encodeURIComponent(x.employee||''),id=encodeURIComponent(x.id||'');return '<div class="dg522-issue '+cls+'"><div class="dg522-issue-head">'+icon+' '+esc522(x.date?date522(x.date)+' · ':'')+esc522(x.title||'Hinweis')+' · '+esc522(status)+'</div>'+(x.detail?'<div class="dg522-issue-detail">'+esc522(x.detail)+'</div>':'')+'<div class="dg522-issue-actions">'+(x.date?'<button class="btn secondary" type="button" onclick="return dg522OpenIssueDay(\''+emp+'\',\''+esc522(x.date)+'\')">Tag öffnen</button>':'')+(!x.reviewed?'<button class="btn success" type="button" onclick="return dg522ReviewOne(\''+id+'\',\''+emp+'\',\''+esc522(x.date||'')+'\')">✓ Geprüft – korrekt</button>':'')+'</div></div>'}
function renderEmployeeReview522(emp){const name=String(emp?.employee||''),box=employeeBox522(name);if(!box)return;const issues=employeeIssues522(name);updateHeader522(box,emp,issues);let panel=box.querySelector(':scope > .dg522-review-panel');if(!panel){panel=document.createElement('div');panel.className='dg522-review-panel';const head=box.querySelector(':scope > .dg521-employee-head');if(head)head.insertAdjacentElement('afterend',panel);else box.insertBefore(panel,box.firstChild)}const openIssues=issues.filter(x=>!x.reviewed),reviewedIssues=issues.filter(x=>x.reviewed);let html='<div class="dg522-review-title">Schnellprüfung '+esc522(name)+'</div><div class="dg522-review-note">Rot und Orange bedeuten: bitte bewusst kontrollieren. Jede Auffälligkeit kann anschließend mit „Geprüft – korrekt“ freigegeben werden. Die vollständige Lohn-/Monatsprüfung erfolgt separat im Monatsabschluss.</div>';if(!issues.length)html+='<div class="status ok">🟢 Keine Auffälligkeiten in den geladenen Tagesabschlüssen.</div>';else{html+='<div class="muted small">'+issues.length+' Prüfposition'+(issues.length===1?'':'en')+': '+openIssues.length+' offen · '+reviewedIssues.length+' bereits geprüft.</div><div class="dg522-review-list">'+issues.map(issueHtml522).join('')+'</div><div class="dg522-release">';if(openIssues.length)html+='<button class="btn success" type="button" onclick="return dg522ReleaseEmployee(\''+encodeURIComponent(name)+'\')">✓ Alle Auffälligkeiten geprüft – Mitarbeiter freigeben</button>';else html+='<div class="dg522-release-ok">✓ Mitarbeiter geprüft / freigegeben</div>';html+='</div>'}panel.innerHTML=html}
function renderAll522(){(lastRows522||[]).forEach(renderEmployeeReview522)}
async function refreshAudit522(rows){if(rows)lastRows522=rows||[];audit522={issues:await buildLocalIssues522(lastRows522)};renderAll522()}
...
window.dg522ReviewOne=async function(issueEncoded,empEncoded,date){const issueId=decodeURIComponent(issueEncoded),employee=decodeURIComponent(empEncoded),p=period522();try{await api(chefPayload({action:'markPayrollIssueReviewed',issueId,targetEmployee:employee,year:p.year,month:p.month,date:date||'',note:'Geprüft – korrekt in Mitarbeiter-Tagesübersicht'}));rememberReviewed522(issueId);await refreshAudit522();if(typeof d3Notice==='function')d3Notice('✓ Auffälligkeit als geprüft gespeichert.','ok')}catch(e){if(typeof d3Notice==='function')d3Notice(e.message,'error');else alert(e.message)}return false};
...
function boot522(){addCss522();installMonthSelectors522();document.querySelectorAll('.dg522-review-panel').forEach(x=>x.remove());document.title='DG Zeiterfassung 10.0';document.querySelectorAll('.login-card .muted.small').forEach(x=>{if(/^Version /.test((x.textContent||'').trim()))x.textContent='Version '+V522});document.querySelectorAll('.hero strong').forEach(x=>{if(/Zeiterfassung/.test(x.textContent||''))x.textContent='Zeiterfassung - '+V522});try{if(window.DG3)DG3.version=V522;window.DG_APP_VERSION=V522}catch(_e){}setTimeout(installMonthSelectors522,500)}
...
function stamp525(){document.title='DG Zeiterfassung 10.0';document.querySelectorAll('.login-card .muted.small').forEach(x=>{if(/^Version /.test((x.textContent||'').trim()))x.textContent='Version '+V525});document.querySelectorAll('.hero strong').forEach(x=>{if(/Zeiterfassung/.test(x.textContent||''))x.textContent='Zeiterfassung - '+V525});try{DG3.version=V525;window.DG_APP_VERSION=V525}catch(_e){}}
...
function stamp(){document.title='DG Zeiterfassung 10.0';document.querySelectorAll('.login-card .muted.small').forEach(x=>{if(/^Version /.test((x.textContent||'').trim()))x.textContent='Version '+V;});document.querySelectorAll('.hero strong').forEach(x=>{if(/Zeiterfassung/.test(x.textContent||''))x.textContent='Zeiterfassung - '+V;});try{DG3.version=V;window.DG_APP_VERSION=V;}catch(_e){}}
...
/* Regie: keine N+1-getObjectReports-Nachladung mehr. GS 8.0 liefert die Historie komplett. */
...
const setView=v=>{try{sessionStorage.setItem(VIEW,v);localStorage.setItem(VIEW,v);}catch(_e){}},getView=()=>{try{return sessionStorage.getItem(VIEW)||localStorage.getItem(VIEW)||sessionStorage.getItem('dg530_view')||localStorage.getItem('dg530_view')||'';}catch(_e){return '';}},setState=(id,child)=>{try{sessionStorage.setItem(STATE,JSON.stringify({id:id||'',child:child||''}));}catch(_e){}},getState=()=>{try{return JSON.parse(sessionStorage.getItem(STATE)||sessionStorage.getItem('dg530_office_state')||'{}')||{};}catch(_e){return {};}};
...
/* ===== CLEAN SOURCE: app-6.0.3-backend.js ===== */
...
/* ===== CLEAN SOURCE: app-6.0-ui.js ===== */
/* DG Zeiterfassung 9.0 - Monteur-Eingabe: Ja/Nein-Tasten und Sprache-zu-Text */
(function(){
'use strict';
...
/* ===== CLEAN SOURCE: app-6.0.2-reminders.js ===== */
...
/* ===== CLEAN SOURCE: app-6.0.3-final.js ===== */
...
/* ===== CLEAN SOURCE: app-6.0.3-reminder-notes.js ===== */
/* DG Zeiterfassung 9.0 - Interne Notizen fuer eigene Reminder */
(function(){
'use strict';
...
/* ===== CLEAN SOURCE: app-7.0-shopping.js ===== */
...
function decorateBossStatusDays72(rows){
  ...
  (rows||[]).forEach(emp=>(emp.days||[]).forEach(day=>{
    ...
    if(detail&&!detail.querySelector('.dg72-report-status')){
      ...
      const credit=(String(day.status)==='Unerlaubte Abwesenheit'||String(day.status)==='Unentschuldigte Abwesenheit')?0:Number(day.creditedHours||0);
      x.textContent=day.status+' · '+(typeof formatHours==='function'?formatHours(credit):Number(credit).toFixed(2))+' Std. automatisch '+(credit>0?'gutgeschrieben':'ohne Gutschrift');
      detail.prepend(x);
    }
  }));
}
...
/* ===== CLEAN SOURCE: app-7.2.1-offer-flow.js ===== */
...
/* ===== CLEAN SOURCE: app-7.4-current.js ===== */
/* DG 8.0 CURRENT ONLY - zusammengefuehrter aktueller Stand */
...
function stamp733(){ /* CLEAN 8.0: Versionsanzeige nur im aktuellen Runtime-Block */ }
...
function stamp734(){ /* CLEAN 8.0: Versionsanzeige nur im aktuellen Runtime-Block */ }
function install734(){
...
function stamp735(){ /* CLEAN 8.0: Versionsanzeige nur im aktuellen Runtime-Block */ }
function install735(){
  patchText735();
...
function payrollTable(rows){
  ...
  return h+'</tbody></table></div><div class="muted small">* Tatsächlich geleistete bzw. gutgeschriebene Stunden werden ausbezahlt.</div>';
}
...
if(typeof baseRunAudit742==='function'&&!window.__DG742_PAYROLL_AUDIT_WRAP){
  ...
  window.dg520RunAudit=async function(){
    ...
    try{
      ...
      console.warn('DG 8.0 Payroll-Nachlauf',e);
    }
    return r;
  };
}

function observePayroll(){ /* CLEAN 8.0: kein dauerhafter DOM-Observer mehr */ }
...
async function raw742(payload){
  ...
  try{
    const r=await fetch(URL742,{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify(Object.assign({},payload,{clientVersion:'9.0'})),signal:ctl.signal});
    if(!r.ok)throw new Error('HTTP '+r.status);
...
function installOffice742(){
  ...
  try{
    ...
    console.error('DG Büroaufbau 9.0 fehlgeschlagen',e);
...
/* ===== DG APP 10 FRONTEND ENHANCEMENTS ===== */
(function(){
'use strict';
const q=id=>document.getElementById(id);
...
function cleanup10(){['adminTimeBankEmployee','adminTimeBankBalance','adminTimeBankHours','adminTimeBankReason','adminTimeBankAction','adminTimeBankStatus','timeBankAdminStatus','timeBankHistory','employeeTimeBank'].forEach(id=>q(id)?.remove());document.querySelectorAll('.admin-section').forEach(s=>{if(/Zeitkonto|Zeitguthaben/i.test(String(s.textContent||'')))s.remove();});document.querySelectorAll('option').forEach(o=>{const t=String(o.textContent||'').trim();if(t==='Freizeitausgleich'||t==='Unentschuldigte Abwesenheit')o.remove();});const sel=q('absenceType');if(sel&&!Array.from(sel.options).some(o=>o.value==='Unerlaubte Abwesenheit')){const o=document.createElement('option');o.value='Unerlaubte Abwesenheit';o.textContent='Unerlaubte Abwesenheit';sel.appendChild(o);}}

/* 7. Mitarbeiter-Gesamtübersicht */
function ensureEmployeeOverview10(){if(q('dg10EmpOverview'))return;const host=q('d3EmployeeAdmin')||document.querySelector('#bossView .card');if(!host)return;const box=document.createElement('div');box.id='dg10EmpOverview';box.className='dg10-panel';box.innerHTML='<h3>Mitarbeiter-Gesamtübersicht</h3><div class="grid2"><div><label>Mitarbeiter</label><select id="dg10EmpSelect"><option value="">Bitte wählen</option></select></div><div style="align-self:end"><button class="btn primary" id="dg10EmpLoad">Übersicht laden</button></div></div><div id="dg10EmpResult" style="margin-top:10px"></div>';host.appendChild(box);q('dg10EmpLoad').onclick=loadEmp10;q('dg10EmpSelect').onchange=loadEmp10;refreshEmpSelect10();}
function refreshEmpSelect10(){const sel=q('dg10EmpSelect');if(!sel)return;const old=sel.value,rows=window.__employeeAdminRows||[];sel.innerHTML='<option value="">Bitte wählen</option>'+rows.map(x=>'<option value="'+esc10(x.name)+'">'+esc10(x.name)+'</option>').join('');if(rows.some(x=>x.name===old))sel.value=old;}
...
function decorateInquiryMerge10(){const list=q('d3InquiryList');if(!list)return;const cards=[...list.querySelectorAll('.report-card')];cards.forEach((card,i)=>{const r=(window.DG3&&DG3.inquiries||[]).filter(x=>x.source!=='AQON PURE')[i];if(!r||card.querySelector('.dg10-inq-merge'))return;const lab=document.createElement('label');lab.className='d3-selection dg10-inq-merge';lab.innerHTML='<input type="checkbox"> Zum Zusammenführen markieren';const cb=lab.querySelector('input');cb.onchange=()=>{const ids=(r.mergedIds&&r.mergedIds.length?r.mergedIds:[r.id]);ids.forEach(id=>cb.checked?mergeIds10.add(id):mergeIds10.delete(id));updateMergeBar10();};card.insertBefore(lab,card.firstChild);if(Number(r.mergedCount||1)>1){const badge=document.createElement('div');badge.className='status info';badge.textContent=String(r.mergedCount)+' Anfragen automatisch zusammengeführt · '+(r.sources||[]).join(' + ');lab.after(badge);}});let bar=q('dg10MergeBar');if(!bar){bar=document.createElement('div');bar.id='dg10MergeBar';bar.className='dg10-mergebar';bar.innerHTML='<button class="btn success" id="dg10MergeBtn" disabled>Markierte Anfragen zusammenführen</button><span id="dg10MergeCount" class="muted small"></span>';list.parentElement.insertBefore(bar,list);q('dg10MergeBtn').onclick=mergeSelected10;}updateMergeBar10();}
function updateMergeBar10(){const n=mergeIds10.size,b=q('dg10MergeBtn'),t=q('dg10MergeCount');if(b)b.disabled=n<2;if(t)t.textContent=n?String(n)+' Anfrage(n) markiert':'';}
async function mergeSelected10(){if(mergeIds10.size<2)return;if(!confirm('Markierte Anfragen zusammenführen?'))return;await api(bossPayload10({action:'mergeCustomerInquiriesV10',ids:[...mergeIds10]}));mergeIds10.clear();await d3Inquiries();}

/* 12. Termin vereinbart = Laufend */
const oldOrders10=window.d3Orders;
if(typeof oldOrders10==='function')window.d3Orders=async function(){await oldOrders10.apply(this,arguments);document.querySelectorAll('#d3OrderPlan .report-card').forEach(card=>{const badge=card.querySelector('.badge');if(!badge)return;const st=String(badge.textContent||'').trim();if(st==='Laufend'){[...card.querySelectorAll('button')].filter(b=>/Termin vereinbaren/i.test(b.textContent||'')).forEach(b=>{b.disabled=true;b.textContent='Termin wurde vereinbart';b.classList.remove('primary');b.classList.add('secondary');});}});};
const oldAppointment10=window.d3Appointment;
if(typeof oldAppointment10==='function')window.d3Appointment=async function(kind,id,offer){const r=kind==='inquiry'?d3Inquiry(id):(window.DG3&&DG3.orders||[]).find(x=>x.id===id);if(!r)throw new Error('Quelle nicht mehr vorhanden.');const workers=(await api(bossPayload10({action:'getPlannerWorkers'}))).filter(x=>x.active),eventId='KT-'+uid();d3Form(offer?'Besichtigung planen':'Termin vereinbaren',[{name:'customer',label:'Kunde',required:true},{name:'address',label:'Vollstaendige Adresse',required:true},{name:'task',label:'Taetigkeit',type:'textarea',required:true},{name:'date',label:'Datum',type:'date',required:true},{name:'start',label:'Von',type:'time',required:true},{name:'end',label:'Bis',type:'time',required:true},{name:'employeeIds',label:'Mitarbeiter',type:'workers',options:workers.map(w=>({value:w.id,label:w.displayName||w.employeeName}))}],{customer:r.customer,address:d3Address(r),task:(offer?'Besichtigung / Angebot: ':'')+(r.description||r.subject||''),date:localDate(),start:'08:00',end:'09:00',employeeIds:[]},async item=>{if(!item.employeeIds.length||item.end<=item.start)throw new Error('Mitarbeiter und Von/Bis pruefen.');await api(bossPayload10({action:'planRequest3',kind,id,offer:!!offer,item:{...item,id:eventId}}));if(kind==='order'&&!offer)await api(bossPayload10({action:'setManualOrderStatus',id,status:'Laufend'}));if(kind==='inquiry')await d3Inquiries();else await d3Orders();});};

/* 8. Abschluss -> Rechnung zu erstellen */
...
async function loadMaps10(){if(window.google&&google.maps&&google.maps.places)return true;if(maps10)return maps10;maps10=(async()=>{const k=await mapsKey10();if(!k)return false;await new Promise((ok,fail)=>{const cb='dg10MapsCb'+Date.now();window[cb]=()=>{delete window[cb];ok();};const sc=document.createElement('script');sc.async=true;sc.defer=true;sc.src='https://maps.googleapis.com/maps/api/js?key='+encodeURIComponent(k)+'&libraries=places&language=de&region=DE&callback='+cb;sc.onerror=fail;document.head.appendChild(sc);});return true;})().catch(()=>false);return maps10;}
