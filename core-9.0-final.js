...
function openMain(){const a=auth();$('loginScreen')?.classList.add('hidden');$('mainScreen')?.classList.remove('hidden');if($('employeeLabel'))$('employeeLabel').textContent='Angemeldet: '+a.employee;if($('date'))$('date').value=localDate();const bossAllowed=canAccessBoss();$('bossTab')?.classList.toggle('hidden',!bossAllowed);let savedView='';try{savedView=sessionStorage.getItem('dg60_view')||localStorage.getItem('dg60_view')||sessionStorage.getItem('dg530_view')||localStorage.getItem('dg530_view')||'';}catch(_e){}if(bossAllowed&&savedView==='boss')showBoss();else showEmployee();DG3.reports={};DG3.inquiries=[];DG3.orders=[];$('regieResult')?.replaceChildren();$('d3RunningList')?.replaceChildren();d3CheckBackend();loadEmployeeDirectory();requestAnimationFrame(()=>{if(customerPad)customerPad.resize();if(employeePad)employeePad.resize()});updateConnection();loadDay();loadCalendarEvents();setTimeout(d35InstallInspectionButton,0);}
