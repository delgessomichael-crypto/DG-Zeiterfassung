const CACHE_NAME='dg-zeiterfassung-5-3-0-office-20260916a';
const OFFER_PATCH='./app-5.2.6-offers.js?v=20260916b';
const REGIE_PATCH='./app-5.2.7-regie.js?v=20260916a';
const OFFICE_PATCH='./app-5.3.0-office-state.js?v=20260916a';
const APP_SHELL=["./","./index.html","./app-5.0.js?v=e3c82c090f11","./app-5.0.css?v=799b73f6d5b0","./manifest.json","./dg_icon_192.png","./dg_icon_512.png",OFFER_PATCH,REGIE_PATCH,OFFICE_PATCH];

async function dg530PatchedHtml(response){
  ...
  if(!html.includes('app-5.3.0-office-state.js'))html=html.replace('</body>','<script defer src="'+OFFICE_PATCH+'"></script></body>');
...
if(e.request.mode==='navigate'){e.respondWith((async()=>{try{return await dg530PatchedHtml(await fetch(e.request,{cache:'no-store'}));}catch(_){const c=await caches.open(CACHE_NAME),hit=await c.match(new URL('./index.html',base).href)||await c.match(new URL('./',base).href);return hit?await dg530PatchedHtml(hit):Response.error()}})());return;}
