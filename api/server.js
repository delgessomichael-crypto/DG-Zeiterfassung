...
// Legacy Apps-Script backend ping removed after final Railway cutover.
...
async function directAiAssistantV10(body){
  ...
  const upstream=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+apiKey},body:JSON.stringify({model,input:[
    {role:'system',content:[{type:'input_text',text:'Du bist Frag DG, der interne Büro-Assistent der Del Gesso Gebäudetechnik. Nutze nur den bereitgestellten App-Kontext für betriebsinterne Fakten. Unterstütze bei Überblick, Priorisierung, Übersetzung, Textentwürfen und Vorbereitung. Führe keine externen oder irreversiblen Aktionen aus. Antworte standardmäßig auf Deutsch; wenn der Nutzer ausdrücklich eine andere Sprache verlangt, antworte in dieser Sprache. Sei klar, knapp und praxisnah. Antworte AUSSCHLIESSLICH als gültiges JSON ohne Markdown im Format {"answer":"kurze Antwort oder Einleitung","tasks":[{"title":"konkretes To-do","detail":"kurzer Grund oder nächster Schritt","sourceType":"inquiry|offer|order","sourceId":"exakt aus dem App-Kontext","stage":"Status aus dem App-Kontext","priority":"rot|gelb|grün","category":"Heute fällig|Wartet auf Kunde|Angebot zu erstellen|Termin fehlt|Sonstiges"}]}. Wenn du konkrete Vorgänge aus dem App-Kontext als To-dos nennst, führe sie in tasks auf und wiederhole die komplette Liste nicht zusätzlich in answer. Verwende sourceId nur exakt aus dem bereitgestellten Kontext und erfinde keine IDs. Setze priority nachvollziehbar: rot nur bei echter Dringlichkeit/heute/überfällig/blockierend, gelb bei zeitnah nötiger Bearbeitung, grün bei normaler Bearbeitung. Setze category nur wenn sie aus Status oder Inhalt klar ableitbar ist, sonst Sonstiges. Status \'Zu erstellen\' ist in der Regel \'Angebot zu erstellen\'; Aufträge mit \'Ohne Termin\' oder \'Termin zu vereinbaren\' sind \'Termin fehlt\'. Maximal 12 tasks. Wenn keine direkt verknüpfbaren Vorgänge nötig sind, verwende tasks:[] und liefere die normale Antwort in answer.'}]},
  ...
  if(parsed&&typeof parsed==='object'&&!Array.isArray(parsed)){
    ...
    const tasks=(Array.isArray(parsed.tasks)?parsed.tasks:[]).slice(0,12).map(t=>{
      ...
      return {
        ...
        priority:['rot','gelb','grün'].includes(String(t&&t.priority||'').toLowerCase())?String(t.priority).toLowerCase():'grün',
        category:String(t&&t.category||'Sonstiges').trim().slice(0,80)
      };
    }).filter(Boolean);
...
async function tryDirectPostgresWrite(action,body){
  ...
  try{
    ...
    if(action==='addRegieAttachments'){
    ...
    }else if(['moveOfferBackToCreate','declineOfferFromReminder','acceptOfferFromReminder','acceptOfferAsRunning','discardOfferPermanently','restoreDiscardedOfferV10','purgeDiscardedOfferV10','setRegieReportsOfferStatus'].includes(action)){
      ...
      else if(action==='restoreDiscardedOfferV10'){targetStatus='Angebot zu erstellen';}
      else if(action==='purgeDiscardedOfferV10'){targetStatus='Endgültig gelöscht';}
