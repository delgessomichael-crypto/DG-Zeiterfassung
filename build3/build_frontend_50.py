...
js=base_js.read_text();css=base_css.read_text();feat=(R/'src/features_5_0.js').read_text();feat_css=(R/'src/features_5_0.css').read_text()
...
# Einheitliche Versionsnummern und Backend-Kompatibilitaet 5.0.
js=js.replace("version:'3.9'","version:'5.0'").replace("DG_APP_VERSION='3.9'","DG_APP_VERSION='5.0'").replace("clientVersion:'3.9'","clientVersion:'5.0'")
js=js.replace('App 3.9','App 5.0').replace('Google-GS 3.0','Google-GS 5.0').replace('Backend 3.0','Backend 5.0')
js=js.replace('/^3\\./','/^(?:3\\.|5\\.)/')
css=css+'\n'+feat_css+'\n'
(O/'app-5.0.js').write_text(js);(O/'app-5.0.css').write_text(css)
html=BeautifulSoup(base_html.read_text(),'html.parser');html.title.string='DG Zeiterfassung 5.0';login=html.select_one('.login-card .center.muted.small');hero=html.select_one('.hero .head-row strong')
if login:login.string='Version 5.0'
if hero:hero.string='Zeiterfassung - 5.0'
