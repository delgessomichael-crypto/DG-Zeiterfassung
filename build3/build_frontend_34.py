...
subprocess.check_call(['python',str(R/'build_frontend_33.py')],env={**os.environ,'DG_BUILD_ROOT':str(R)})
js=(O/'app-3.3.js').read_text();feat=(R/'src/features_3_4.js').read_text();css=(O/'app-3.3.css').read_text()+'\n'+(R/'src/features_3_4.css').read_text()
js=js.replace("version:'3.3'","version:'3.4'",1).replace("DG_APP_VERSION='3.3'","DG_APP_VERSION='3.4'",1).replace("clientVersion:'3.3'","clientVersion:'3.4'",1).replace('App 3.3','App 3.4')
js=js.replace("parts[0]===3&&parts[1]>=2","parts[0]===3&&parts[1]>=3").replace('App 3.3 benötigt Google-GS Backend 3.2 oder neuer.','App 3.4 benötigt Google-GS Backend 3.3 oder neuer.')
marker='function d3Startup()'
if marker not in js: raise SystemExit('startup marker missing')
js=js.replace(marker,feat+'\n\n'+marker,1)
(O/'app-3.4.js').write_text(js);(O/'app-3.4.css').write_text(css)
html=BeautifulSoup((O/'index.html').read_text(),'html.parser');html.title.string='DG Zeiterfassung 3.4'
login=html.select_one('.login-card .center.muted.small');hero=html.select_one('.hero .head-row strong')
if login:login.string='Version 3.4'
if hero:hero.string='Zeiterfassung - 3.4'
