...
window.DG3={version:'3.5.2',backend:'',pending:0,reads:new Map(),reports:{},active:'Abgeschlossen',loaders:{},ready:false};window.DG_APP_VERSION='3.5.2';
...
async function d34AqonInquiries(){setMessage('d34AqonStatus','AQON PURE Posteingang wird abgeglichen ...','info');try{const now=Date.now();if(!DG3.aqonSyncAt||now-DG3.aqonSyncAt>30000){const sync=await api(chefPayload({action:'syncCustomerInquiries'}));DG3.aqonSyncAt=now;if(sync.aqonReplyErrors||sync.gmailFileErrors)setMessage('d34AqonStatus','AQON-Abgleich abgeschlossen, aber mit '+Number(sync.aqonReplyErrors||0)+' Antwort- und '+Number(sync.gmailFileErrors||0)+' Gmail-Ablagefehler(n).','warn');}const all=await api(chefPayload({action:'getCustomerInquiries',status:'Offen'}));DG3.inquiries=all;const rows=all.filter(r=>r.source==='AQON PURE');$('d34AqonList').innerHTML=rows.map((r,i)=>d32InquiryCard(r,i,false)).join('')||'Keine offenen AQON PURE Anfragen.';setMessage('d34AqonStatus',rows.length+' offene AQON PURE Anfrage(n).','ok');d34SetAqonCount(rows.length);d3Count('inquiries',all.length);}catch(e){setMessage('d34AqonStatus','AQON-Abgleich fehlgeschlagen: '+e.message,'error');}}
