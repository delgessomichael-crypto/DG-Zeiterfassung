...
base_js=ROOT/'app-3.5.js';base_css=ROOT/'app-3.5.css';base_html=ROOT/'index.html';base_manifest=ROOT/'manifest.json';payroll_js=ROOT/'app-5.2.0-payroll.js'
for p in [base_js,base_css,base_html,base_manifest,payroll_js]:
...
versions=['3.9','5.0','5.0.1','5.0.2','5.0.3','5.0.4','5.0.5','5.1','5.1.0','5.1.1','5.1.2','5.2.0']
for old in versions: js=js.replace("version:'"+old+"'","version:'5.2.0'")
for old in versions: js=js.replace("DG_APP_VERSION='"+old+"'","DG_APP_VERSION='5.2.0'")
for old in versions: js=js.replace("clientVersion:'"+old+"'","clientVersion:'5.2.0'")
for old in ['App 3.9','App 5.0.1','App 5.0.2','App 5.0.3','App 5.0.4','App 5.0.5','App 5.1','App 5.1.1','App 5.1.2']: js=js.replace(old,'App 5.2.0')
for old in ['Google-GS 3.0','Google-GS 5.0.1','Google-GS 5.0.2','Google-GS 5.1','Google-GS 5.1.1','Google-GS 5.1.2']: js=js.replace(old,'Google-GS 5.2.0')
for old in ['Backend 3.0','Backend 5.0.2','Backend 5.1','Backend 5.1.1','Backend 5.1.2']: js=js.replace(old,'Backend 5.2.0')
js=js.replace('/^3\\./','/^(?:3\\.|5\\.)/')
css=css+'\n'+feat_css+'\n'
(O/'app-5.0.js').write_text(js);(O/'app-5.0.css').write_text(css)
html=BeautifulSoup(base_html.read_text(),'html.parser');html.title.string='DG Zeiterfassung 5.2.0';login=html.select_one('.login-card .center.muted.small');hero=html.select_one('.hero .head-row strong')
if login:login.string='Version 5.2.0'
if hero:hero.string='Zeiterfassung - 5.2.0'
