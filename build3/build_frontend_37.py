...
for p in [base_js,base_css,base_html,base_manifest]:
    if not p.exists(): raise SystemExit('stable 3.6 base missing: '+str(p))
js=base_js.read_text();css=base_css.read_text();feat=''.join((R/'src'/f'features_3_7_{i}.js').read_text() for i in (1,2,3));feat_css=(R/'src/features_3_7.css').read_text()
...
js=js.replace("version:'3.6'","version:'3.7'",1).replace("DG_APP_VERSION='3.6'","DG_APP_VERSION='3.7'",1).replace("clientVersion:'3.6'","clientVersion:'3.7'",1).replace('App 3.6','App 3.7')
css=css+'\n'+feat_css+'\n'
(O/'app-3.5.js').write_text(js);(O/'app-3.5.css').write_text(css)
html=BeautifulSoup(base_html.read_text(),'html.parser');html.title.string='DG Zeiterfassung 3.7';login=html.select_one('.login-card .center.muted.small');hero=html.select_one('.hero .head-row strong')
if login:login.string='Version 3.7'
if hero:hero.string='Zeiterfassung - 3.7'
