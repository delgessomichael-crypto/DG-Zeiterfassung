...
assert 'DG Zeiterfassung 5.0.1' in html
assert 'Version 5.0.1' in html
assert 'Zeiterfassung - 5.0.1' in html
assert "version:'5.0.1'" in js
assert "DG_APP_VERSION='5.0.1'" in js
assert "clientVersion:'5.0.1'" in js
assert 'Google-GS 5.0.1' in js and 'Google-GS 3.0' not in js
assert '/^(?:3\\.|5\\.)/' in js
...
assert 'dg-zeiterfassung-5-0-1-' in sw
assert 'DG Zeiterfassung 5.0.1' in manifest and 'DG 5.0.1' in manifest
...
print('DG 5.0.1 verification OK')
