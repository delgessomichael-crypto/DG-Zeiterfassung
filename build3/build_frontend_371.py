...
for p in [base_js,base_css,base_html,base_manifest]:
    if not p.exists(): raise SystemExit('stable 3.7 base missing: '+str(p))
...
js=js.replace("version:'3.7'","version:'3.7.1'",1).replace("DG_APP_VERSION='3.7'","DG_APP_VERSION='3.7.1'",1).replace("clientVersion:'3.7'","clientVersion:'3.7.1'",1).replace('App 3.7','App 3.7.1')
css=css+'\n'+feat_css+'\n'
(O/'app-3.5.js').write_text(js);(O/'app-3.5.css').write_text(css)
html=BeautifulSoup(base_html.read_text(),'html.parser');html.title.string='DG Zeiterfassung 3.7.1';login=html.select_one('.login-card .center.muted.small');hero=html.select_one('.hero .head-row strong')
if login:login.string='Version 3.7.1'
if hero:hero.string='Zeiterfassung - 3.7.1'
