...
js=base_js.read_text();css=base_css.read_text();feat='\n'.join((R/'src'/name).read_text() for name in features);feat_css=(R/'src/features_5_0.css').read_text()+'\n'+(R/'src/features_5_0_1.css').read_text()
...
versions=['3.9','5.0','5.0.1','5.0.2','5.0.3','5.0.4','5.0.5','5.1','5.1.0','5.1.1','5.1.2']
for old in versions: js=js.replace("version:'"+old+"'","version:'5.1.2'")
for old in versions: js=js.replace("DG_APP_VERSION='"+old+"'","DG_APP_VERSION='5.1.2'")
for old in versions: js=js.replace("clientVersion:'"+old+"'","clientVersion:'5.1.2'")
for old in ['App 3.9','App 5.0.1','App 5.0.2','App 5.0.3','App 5.0.4','App 5.0.5','App 5.1','App 5.1.1']: js=js.replace(old,'App 5.1.2')
for old in ['Google-GS 3.0','Google-GS 5.0.1','Google-GS 5.0.2','Google-GS 5.1','Google-GS 5.1.1']: js=js.replace(old,'Google-GS 5.1.2')
for old in ['Backend 3.0','Backend 5.0.2','Backend 5.1','Backend 5.1.1']: js=js.replace(old,'Backend 5.1.2')
js=js.replace('/^3\\./','/^(?:3\\.|5\\.)/')
css=css+'\n'+feat_css+'\n'
(O/'app-5.0.js').write_text(js);(O/'app-5.0.css').write_text(css)
html=BeautifulSoup(base_html.read_text(),'html.parser');html.title.string='DG Zeiterfassung 5.1.2';login=html.select_one('.login-card .center.muted.small');hero=html.select_one('.hero .head-row strong')
if login:login.string='Version 5.1.2'
if hero:hero.string='Zeiterfassung - 5.1.2'
