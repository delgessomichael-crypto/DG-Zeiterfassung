const CACHE_NAME = 'dg-zeiterfassung-v48';
...
async function withV48Patches(response) {
  ...
  html = html.replace(/<title>DG Zeiterfassung v\d+<\/title>/, '<title>DG Zeiterfassung v48</title>');
...
self.addEventListener('fetch', event => {
  ...
  event.respondWith((async () => {
    try {
      ...
      const served = isPage ? await withV48Patches(response) : response;
      if (served && served.ok) {
      ...
      if (event.request.mode === 'navigate') {
        ...
        return fallback ? withV48Patches(fallback) : fallback;
      }
      throw err;
    }
  })());
