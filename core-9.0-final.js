...
function cleanup10(){['adminTimeBankEmployee','adminTimeBankBalance','adminTimeBankHours','adminTimeBankReason','adminTimeBankAction','adminTimeBankStatus'].forEach(id=>q(id)?.classList.add('dg10-hide'));document.querySelectorAll('option').forEach(o=>{if(String(o.textContent||'').trim()==='Freizeitausgleich')o.remove();});const sel=q('absenceType');if(sel&&!Array.from(sel.options).some(o=>o.value==='Unentschuldigte Abwesenheit')){const o=document.createElement('option');o.value='Unentschuldigte Abwesenheit';o.textContent='Unentschuldigte Abwesenheit';sel.appendChild(o);}}
...
if(typeof oldOrders10==='function')window.d3Orders=async function(){await oldOrders10.apply(this,arguments);document.querySelectorAll('#d3OrderPlan .report-card').forEach(card=>{const badge=card.querySelector('.badge');if(!badge)return;const st=String(badge.textContent||'').trim();if(st==='Laufend'){[...card.querySelectorAll('button')].filter(b=>/Termin vereinbaren/i.test(b.textContent||'')).forEach(b=>{b.disabled=true;b.textContent='Termin vereinbart';b.classList.remove('primary');b.classList.add('secondary');});}});};
const oldAppointment10=window.d3Appointment;
if(typeof oldAppointment10==='function')window.d3Appointment=async function(kind,id,offer){const before=(window.DG3&&DG3.orders||[]).find(x=>x.id===id);const r=await oldAppointment10.apply(this,arguments);if(kind==='order'&&!offer&&before){try{await api(bossPayload10({action:'setManualOrderStatus',id,status:'Laufend'}));if(typeof window.d3Orders==='function')await window.d3Orders();}catch(_e){}}return r;};
