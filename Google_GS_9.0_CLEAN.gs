...
function doPost(e) {
  ...
  try {
    ...
    if (action === 'createInquiryReminder') {
      return jsonResponse_({ok:true,data:createInquiryReminder(clean_(data.employee),clean_(data.employeePin),clean_(data.id),Number(data.days)||0)});
    }
