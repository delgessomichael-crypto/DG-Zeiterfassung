...
for x in ['DG Zeiterfassung 5.2.3','Version 5.2.3','Zeiterfassung - 5.2.3']: assert x in html
assert "version:'5.2.3'" in js and "DG_APP_VERSION='5.2.3'" in js and "clientVersion:'5.2.3'" in js
assert 'Google-GS 5.2.0.2' in js
for x in ['Monatsabschluss &amp; Lohnübergabe','getMonthPayrollAudit','setPayrollMonthStatus','updateBossDayEntry','Lohnübergabe','Eintrag entfernen & Stunden abziehen','Überprüfung notwendig','dg521-employee-head','dg521ConfirmDelete','Schnellprüfung','Alle Hinweise geprüft – Mitarbeiter freigeben','dg522ReviewOne','dg522ReleaseEmployee']: assert x in js
assert 'Prüfhinweise werden geladen' not in js
assert 'dg-zeiterfassung-5-2-3-' in sw
assert 'DG Zeiterfassung 5.2.3' in manifest and 'DG 5.2.3' in manifest
assert html.count('app-5.0.js')==1 and html.count('app-5.0.css')==1
print('DG 5.2.3 verification OK')
