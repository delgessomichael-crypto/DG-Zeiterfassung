...
/* DG Zeiterfassung 7.0 - konsolidierter Runtime-Layer */
(function(){
'use strict';
const V='7.0',VIEW='dg60_view',STATE='dg60_office_state',BACK='dg60_backend',BACK_TTL=1800000,OFFER_TTL=30000;
...
/* Regie: keine N+1-getObjectReports-Nachladung mehr. GS 7.0 liefert die Historie komplett. */
...
/* DG Zeiterfassung 7.0 - strikte, einheitliche Backend-Anbindung */
(function(){
'use strict';
const V='7.0',KEY='dg70_backend',TTL=5*60*1000;
...
function showMismatch(found){window.__DG_FOUND_BACKEND=String(found||'unbekannt');if(exact(found)){clearVersionNotices();return true;}if(window.DG3)DG3.backend='';const msg='Versionsstand stimmt nicht: App '+V+' benötigt Google-GS '+V+'. Aktiv ist Google-GS '+foundVersion()+'.';if(typeof window.d3Notice==='function')window.d3Notice(msg,'warn');return false;}
window.d3CheckBackend=d3CheckBackend=async function(force){
  if(!force){try{const c=JSON.parse(sessionStorage.getItem(KEY)||'null');if(c&&exact(c.version)&&Date.now()-Number(c.ts||0)<TTL){window.__DG_FOUND_BACKEND=V;if(window.DG3)DG3.backend=V;clearVersionNotices();return true;}}catch(_e){}}
  try{
    ...
    try{sessionStorage.removeItem('dg60_backend_reachable');sessionStorage.removeItem('dg60_backend');sessionStorage.removeItem('dg602_backend');if(exact(found))sessionStorage.setItem(KEY,JSON.stringify({ts:Date.now(),version:found}));else sessionStorage.removeItem(KEY);}catch(_e){}
...
window.d3Api=d3Api=async function(payload){
  ...
  const run=(async()=>{
    if(action!=='ping'&&(!window.DG3||String(DG3.backend||'')!==V)){const ok=await d3CheckBackend(true);if(!ok)throw dgError('App '+V+' kann nicht speichern/laden, weil Google-GS '+foundVersion()+' aktiv ist. Benötigt wird Google-GS '+V+'.','version');}
...
/* DG Zeiterfassung 7.0 - Monteur-Eingabe: Ja/Nein-Tasten und Sprache-zu-Text */
(function(){
'use strict';
...
/* DG Zeiterfassung 7.0 - UI-Farben, Kalender-Refresh und zentraler Versionsstempel */
(function(){
'use strict';
const V='7.0';
...
/* DG Zeiterfassung 7.0 - Eigene Reminder mit Sprache, Bildern und Dateien */
(function(){
'use strict';
const V='7.0';
...
/* DG Zeiterfassung 7.0 - finaler Produktions-Hardening-Layer */
(function(){
'use strict';
const V='7.0';
const BACKEND_URL='https://script.google.com/macros/s/AKfycby2L3SMgh2RoGWsNRUp6o11g4iyZ8bgkSIGaAZPnBXCkJTkDDGF9aydn9vVKMB7kXsO/exec';
...
/* DG Zeiterfassung 7.0 - Interne Notizen fuer eigene Reminder */
(function(){
'use strict';
