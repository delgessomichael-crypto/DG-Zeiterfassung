...
window.DG3={version:'5.2.3',backend:'',pending:0,reads:new Map(),reports:{},active:'Abgeschlossen',loaders:{},ready:false};window.DG_APP_VERSION='5.2.3';
...
window.d3CheckBackend=d3CheckBackend=async function(){
  try{const r=await api({action:'ping'}),found=String(r&&r.version||''),p=found.split('.').map(Number),ok=p[0]===5&&(p[1]>0||(p[1]===0&&p[2]>=1));DG3.backend=ok?found:'';if(!ok)d3Notice('App 5.2.3 benötigt Google-GS 5.2.0.2.2 oder neuer. Gefunden: '+(found||'unbekannt')+'. Speichern ist gesperrt.','warn');else $('d3Notice')?.remove();return ok;}catch(e){DG3.backend='';d3Notice('Verbindungsprüfung fehlgeschlagen: '+e.message,'warn');return false;}
};
...
window.d3CheckBackend=d3CheckBackend=async function(force){
  ...
  try{
    ...
    d3Notice('App 5.2.3.1 benötigt Google-GS 5.2.0.2.2.1 oder neuer. Gefunden: '+(found||'unbekannt')+'. Speichern ist gesperrt.','warn');return false;
...
window.d3CheckBackend=d3CheckBackend=async function(force){
  ...
  try{
    ...
    DG3.backend=ok?found:'';
    if(ok){try{sessionStorage.setItem('dg51_backend',JSON.stringify({ts:Date.now(),version:found}));}catch(_e){}$('d3Notice')?.remove();return true;}
    d3Notice('App 5.2.3.2 benötigt Google-GS 5.2.0.2.2.2 oder neuer. Gefunden: '+(found||'unbekannt')+'. Speichern ist gesperrt.','warn');return false;
  }catch(e){DG3.backend='';d3Notice('Verbindungsprüfung fehlgeschlagen: '+e.message,'warn');return false;}
};
...
/* DG Zeiterfassung 5.2.3 - schnelle lokale Mitarbeiter-Pruefung ohne Monatsaudit-Timeout */
(function(){
'use strict';
const V522='5.2.3';
...
async function addIssue522(list,severity,type,employee,date,title,detail,key,extra){const p=period522(),id=await issueId522([p.year,p.month,employee,date||'',type,key||'']),reviewed=severity==='warn'&&reviewedSet522().has(id);list.push(Object.assign({id,severity,type,employee,date:date||'',title,detail:detail||'',reviewed},extra||{}))}
...
function updateHeader522(box,emp,issues){const head=box?.querySelector(':scope > .dg521-employee-head');if(!head)return;const errors=issues.filter(x=>x.severity==='error').length,openWarnings=issues.filter(x=>x.severity==='warn'&&!x.reviewed).length,open=errors+openWarnings,ok=open===0;const lamp=head.querySelector('.dg521-lamp'),label=head.querySelector('.dg521-plausibility'),meta=head.querySelector('.dg521-meta');if(lamp){lamp.classList.toggle('ok',ok);lamp.classList.toggle('bad',!ok)}if(label){label.classList.toggle('ok',ok);label.classList.toggle('bad',!ok);label.textContent=ok?(issues.length?'Geprüft / freigegeben':'Alles plausibel'):'Überprüfung notwendig'}if(meta){const days=Number((emp?.days||[]).length);meta.textContent=days+' Tag'+(days===1?'':'e')+(issues.length?(open?' · '+open+' offen':' · '+issues.length+' geprüft'):'')}head.title=issues.length?issues.map(x=>(x.reviewed&&x.severity==='warn'?'✓ ':'')+(x.date?date522(x.date)+' · ':'')+x.title).join('\n'):'Keine Auffälligkeiten in der Schnellprüfung.'}
function issueHtml522(x){const cls=x.severity==='error'?'error':(x.reviewed?'reviewed':'warn'),icon=x.severity==='error'?'🔴':(x.reviewed?'🟢':'🟠'),status=x.severity==='error'?'Korrektur erforderlich':(x.reviewed?'geprüft – korrekt':'prüfen'),emp=encodeURIComponent(x.employee||''),id=encodeURIComponent(x.id||'');return '<div class="dg522-issue '+cls+'"><div class="dg522-issue-head">'+icon+' '+esc522(x.date?date522(x.date)+' · ':'')+esc522(x.title||'Hinweis')+' · '+esc522(status)+'</div>'+(x.detail?'<div class="dg522-issue-detail">'+esc522(x.detail)+'</div>':'')+'<div class="dg522-issue-actions">'+(x.date?'<button class="btn secondary" type="button" onclick="return dg522OpenIssueDay(\''+emp+'\',\''+esc522(x.date)+'\')">Tag öffnen</button>':'')+(x.severity==='warn'&&!x.reviewed?'<button class="btn success" type="button" onclick="return dg522ReviewOne(\''+id+'\',\''+emp+'\',\''+esc522(x.date||'')+'\')">✓ Geprüft – korrekt</button>':'')+'</div></div>'}
function renderEmployeeReview522(emp){const name=String(emp?.employee||''),box=employeeBox522(name);if(!box)return;const issues=employeeIssues522(name);updateHeader522(box,emp,issues);let panel=box.querySelector(':scope > .dg522-review-panel');if(!panel){panel=document.createElement('div');panel.className='dg522-review-panel';const head=box.querySelector(':scope > .dg521-employee-head');if(head)head.insertAdjacentElement('afterend',panel);else box.insertBefore(panel,box.firstChild)}const errors=issues.filter(x=>x.severity==='error'),warnings=issues.filter(x=>x.severity==='warn'),openWarnings=warnings.filter(x=>!x.reviewed),reviewedWarnings=warnings.filter(x=>x.reviewed);let html='<div class="dg522-review-title">Schnellprüfung '+esc522(name)+'</div><div class="dg522-review-note">Prüft die bereits geladenen Tagesabschlüsse sofort. Die vollständige Lohn-/Monatsprüfung erfolgt separat im Monatsabschluss.</div>';if(!issues.length)html+='<div class="status ok">🟢 Keine Auffälligkeiten in den geladenen Tagesabschlüssen.</div>';else{html+='<div class="muted small">'+issues.length+' Prüfposition'+(issues.length===1?'':'en')+': '+errors.length+' Fehler · '+openWarnings.length+' ungeprüfte Hinweise · '+reviewedWarnings.length+' bereits geprüft.</div><div class="dg522-review-list">'+issues.map(issueHtml522).join('')+'</div><div class="dg522-release">';if(errors.length)html+='<div class="dg522-release-blocked">🔴 Freigabe gesperrt: '+errors.length+' Fehler müssen zuerst korrigiert werden. Hinweise können unabhängig davon bestätigt werden.</div>';else if(openWarnings.length)html+='<button class="btn success" type="button" onclick="return dg522ReleaseEmployee(\''+encodeURIComponent(name)+'\')">✓ Alle Hinweise geprüft – Mitarbeiter freigeben</button>';else html+='<div class="dg522-release-ok">✓ Mitarbeiter geprüft / freigegeben</div>';html+='</div>'}panel.innerHTML=html}
...
window.dg522ReviewOne=async function(issueEncoded,empEncoded,date){const issueId=decodeURIComponent(issueEncoded),employee=decodeURIComponent(empEncoded),p=period522();try{await api(chefPayload({action:'markPayrollIssueReviewed',issueId,targetEmployee:employee,year:p.year,month:p.month,date:date||'',note:'Geprüft – korrekt in Mitarbeiter-Tagesübersicht'}));rememberReviewed522(issueId);await refreshAudit522();if(typeof d3Notice==='function')d3Notice('✓ Hinweis als geprüft gespeichert.','ok')}catch(e){if(typeof d3Notice==='function')d3Notice(e.message,'error');else alert(e.message)}return false};
window.dg522ReleaseEmployee=async function(empEncoded){const employee=decodeURIComponent(empEncoded),issues=employeeIssues522(employee),errors=issues.filter(x=>x.severity==='error'),openWarnings=issues.filter(x=>x.severity==='warn'&&!x.reviewed),p=period522();if(errors.length){if(typeof d3Notice==='function')d3Notice('Freigabe nicht möglich: zuerst die roten Fehler korrigieren.','error');return false}if(!openWarnings.length)return false;if(!confirm(employee+': '+openWarnings.length+' Hinweis'+(openWarnings.length===1?'':'e')+' als geprüft und korrekt bestätigen?'))return false;try{for(const x of openWarnings){await api(chefPayload({action:'markPayrollIssueReviewed',issueId:x.id,targetEmployee:employee,year:p.year,month:p.month,date:x.date||'',note:'Mitarbeiter geprüft / freigegeben in Tagesübersicht'}));rememberReviewed522(x.id)}await refreshAudit522();if(typeof d3Notice==='function')d3Notice('✓ '+employee+' geprüft und freigegeben.','ok')}catch(e){if(typeof d3Notice==='function')d3Notice(e.message,'error');else alert(e.message)}return false};
